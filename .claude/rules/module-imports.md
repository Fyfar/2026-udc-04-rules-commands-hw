...
1. `grep -RnE "from ['\"]\.\.?/[^'\"]+['\"]" app/src --include="*.ts" | grep -vE "\.js['\"]"`
   returns nothing — every relative import ends in `.js`. Matches both quote
   styles, so a single-quoted `from './reducer'` can't slip through.
