...
1. `grep -RnE "(^|[^A-Za-z.])(from|import)[[:space:]]*\(?[[:space:]]*['\"]\.\.?/[^'\"]+['\"]" app/src --include="*.ts" | grep -vE "\.js['\"]"`
   returns nothing — every relative import ends in `.js`. Covers both quote
   styles and every form (`from`, side-effect `import "./x"`, dynamic
   `import("./x")`, re-export `... from "./x"`), so none can slip through.
